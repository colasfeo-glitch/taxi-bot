...
// 1. Servidor web básico para mantener Render despierto
const app = express();
...
async function enviarWhatsApp(telefono, mensaje) {
    try {
        ...
        console.error(`❌ Error al enviar WhatsApp a ${telefono}:`, error.response ? error.response.data : error.message);
    }
}

// 4. EL VIGILANTE DE ESTADOS EN TIEMPO REAL
console.log("👀 Vigilante de estados conectado a Firebase...");
let inicio = Date.now(); // Protección anti-mensajes repetidos al arrancar
...
db.collection('reservations').onSnapshot(snapshot => {
    snapshot.docChanges().forEach(async (change) => {
        ...
        // Ignorar viajes creados en el pasado antes de encender el bot
        ...
        // Creamos los enlaces dinámicos para que el cliente siga el mapa o consulte su cartera
        let currentURL = "https://taxilapobladevallbona.com"; // Dominio base por defecto
        ...
        if (change.type === 'modified') {
            ...
            // 2. EL TAXISTA ENCIENDE EL GPS E INICIA EL VIAJE (ESTADO: EN CAMINO)
            ...
            // 4. EL CONDUCTOR COBRA EL SERVICIO Y CIERRA CAJA (ESTADO: FINALIZADO)
                ...
                if (isVipUser) {
                    // Si pagó usando puntos de la cartera
                    ...
                    // Si pagó normal en efectivo/tarjeta pero generó reembolso
                    else if (earned > 0) {
            ...
            // 5. EL VIAJE SE CANCELA POR LA CENTRAL O FALTA DE TIEMPO (ESTADO: CANCELADO)
